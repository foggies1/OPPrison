...
public class PickaxeStorage {
    ...
    public void updatePickaxe(Player player){
        get(player.getUniqueId()).whenComplete((pickaxe, throwable) -> {
            if(throwable != null){
    ...
    public CompletableFuture<Optional<PlayerPickaxe>> get(UUID uuid){
        return CompletableFuture.supplyAsync(() -> Optional.of(pickaxeMap.get(uuid)));
    }

    public void save(UUID uuid, boolean removeFromCache){

        get(uuid).whenComplete((pickaxe, throwable) -> {
            if(throwable != null){
                throwable.printStackTrace();
                return;
            }
