...
public class SetPrestigeCMD {

    public SetPrestigeCMD(OPPrison plugin) {
        final PlayerStorage playerStorage = plugin.getPlayerStorage();

        Commands.create()
                .assertPermission(Permissions.PRESTIGE_ADMIN.getPermission())
                ...
                .handler(c -> {
                    ...
                    playerStorage.get(target.getUniqueId()).ifPresent(pp -> pp.setPrestige(amount));
                                    ...
                                    .hoverEvent(HoverEvent.showText(
                                            TextComponent.of(
                                                    StringUtils.color("&7Below is information on how your &cPrestige Data" + "\n" +
                                                            "&7has been altered: " + "\n" +
                                                            "" + "\n" +
                                                            "&c&l" + Lang.BLOCK_SYMBOL.getMessage() + "&fPrestige Set To: " + Number.pretty(amount)
                                                    ))
                                    ))
                    );
