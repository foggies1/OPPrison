...
public class TakePrestigeCMD {

    public TakePrestigeCMD(OPPrison plugin) {
        ...
        Commands.create()
                ...
                .handler(c -> {
                    ...
                    playerStorage.get(target.getUniqueId()).ifPresent(pp -> pp.takePrestige(amount));
                                    ...
                                    .hoverEvent(HoverEvent.showText(
                                            TextComponent.of(
                                                    StringUtils.color("&7Below is information on how your &cPrestige Data" + "\n" +
                                                            "&7has been altered: " + "\n" +
                                                            "" + "\n" +
                                                            "&c&l" + Lang.BLOCK_SYMBOL.getMessage() + "&fPrestiges Removed: " + Number.pretty(amount)
                                                    ))
                                    ))
                    );
